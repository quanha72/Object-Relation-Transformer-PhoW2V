...
class BoxMultiHeadedAttention(nn.Module):
    ...
    def forward(self, input_query, input_key, input_value, input_box, mask=None):
        ...
        relative_geometry_Embedding = utils.BoxRelationalEmbedding(input_box, trignometric_embedding= self.trignometric_embedding)
        flatten_relative_geometry_Embedding = relative_geometry_Embedding.view(-1,self.dim_g)

        # 1) Do all the linear projections in batch from d_model => h x d_k
        query, key, value = \
            [l(x).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
             for l, x in zip(self.linears, (input_query, input_key, input_value))]
        box_size_per_head = list(relative_geometry_Embedding.shape[:3])
        box_size_per_head.insert(1, 1)
        relative_geometry_weights_per_head = [l(flatten_relative_geometry_Embedding).view(box_size_per_head) for l in self.WGs]
